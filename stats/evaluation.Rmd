...
rownames(total_mds) <- rownames(embeddings)

category_mds <- total_mds[endsWith(rownames(total_mds), "category"),]
rownames(category_mds) <- rownames(total_mds)[endsWith(rownames(total_mds), "category")]

image_mds <- total_mds[endsWith(rownames(total_mds), ".png"),]
rownames(image_mds) <- rownames(total_mds)[endsWith(rownames(total_mds), ".png")]

literal_mds <- total_mds[grepl("literal",rownames(total_mds), fixed = TRUE),]
rownames(literal_mds) <- rownames(total_mds)[grepl("literal",rownames(total_mds), fixed = TRUE)]

sentimental_mds <- total_mds[grepl("sentimental",rownames(total_mds), fixed = TRUE),]
rownames(sentimental_mds) <- rownames(total_mds)[grepl("sentimental",rownames(total_mds), fixed = TRUE)]
```

```{r}
# K-means clustering
total_clust <- kmeans(total_mds, length(categories))$cluster %>% as.factor()
category_clust <- as.factor(categories) # simply translate categories into numbers for clustering, assuming each category is a cluster.
...
```{r}
clusterPanel <- function (x, y, col = par("col"), bg = NA, pch = par("pch"), 
    cex = 1, col.smooth = 2, span = 2/3, iter = 3, ...) 
{
    points(x, y, pch = pch, col = col, bg = bg, cex = cex, xlab = names(x), ylab = names(y))
...
```{r}
png("plots/clusters_total.png", 1400, 1400, type = 'cairo')
par(xpd = T, mar=c(8,15,4,10) + 0.1)
# plot(total_mds[,-3], 
#        col = coul_16[total_clust],
#        pch=19,
#        cex=1.5,
#        las=1,
#        cex.axis = axisLabelSize,
...
text(category_mds[,1], category_mds[,2]+.2, category_mds[,3],cex=labelSize)

# legend("right",
#     legend = levels(as.factor(categories[total_clust])),
#     pch = 19,
#     cex=legendLabelSize,
#     pt.cex = 3,
#     col = coul_16[total_clust],
...
png("plots/clusters_all.png", 1000, 1000, type = 'cairo')
...
png("plots/clusters_image.png", 1000, 1000, type = 'cairo')
par(mar=c(14, 10, 8, 4) + 0.1)
...
png("plots/clusters_descriptions.png", 1000, 1000, type = 'cairo')
par(mar=c(14, 10, 8, 4) + 0.1)
