...
---
First, we read the descriptions, stats, and annotators data.
```{r}
...
for (title in description_vars){
  ...
  png(sprintf("plots/%s.png",title), 900, 800, type = 'cairo')
  par(mar=plotMargins) 
  ...
  for (title_comp in description_vars){
    ...
    png(sprintf("plots/%s_vs_%s.png",title,title_comp), 900, 800, type = 'cairo')
    par(mar=plotMargins) 
...
png("plots/timeTaken.png", 650, 650, type = 'cairo')
par(mar=plotMargins) 
boxplot(round(literal_stats$timeTaken,1),
    ...
    cex.main = titleSize,
    cex.lab = labelSize)
  text(x=1.25, y=fivenum(round(literal_stats$timeTaken,1)),labels=fivenum(round(literal_stats$timeTaken,1)), font=2)
dev.off()
...
png(sprintf("plots/autoTags_accuracy_vs_F1.png",title), 800, 800, type = 'cairo')
...
png(sprintf("plots/autoTags_precision_vs_recall.png",title), 1000, 1000, type = 'cairo')
...
png(sprintf("plots/autoTags_precision_vs_category.png",title), 1000, 1000, type = 'cairo')
...
png(sprintf("plots/autoTags_recall_vs_category.png",title), 1000, 1000, type = 'cairo')
par(mar=c(8,15,4,2) + 0.1)
...
png(sprintf("plots/autoTags_f1_vs_category.png",title), 1000, 1000, type = 'cairo')
par(mar=c(8,15,4,2) + 0.1)
