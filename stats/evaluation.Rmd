...
pairs = ggpairs(st,
                ...
                columnLabels = c("age \n(years)", "income \n(€ p.A.)", "household \nsize \n(persons)","living area \n(m^2)","gender"),                 lower = list(combo = "dot_no_facet"),
...
write.csv(as.data.frame(prototype_dist_cos),"category_prototype_dist_cos.csv", row.names=TRUE)
```
...
png("plots/category_prototype_eucl_dist.png", 1400, 1400, type = 'cairo')
...
ggplot(eucl_tidied,
  ...
  coord_fixed()+
  labs(title = "Euclidean distance between category prototypes") +
  ...
  theme(legend.title = element_text(size=32),
        ...
        plot.title = element_text(size=48, margin = margin(0,20,20,20)),
        plot.margin = unit(c(2, 2, 2, 2), "cm"),
        axis.title.x = element_text(size=32, margin = margin(40,10,10,10)),
        axis.title.y = element_text(size=32, margin = margin(10,40,10,20)),
...
df_cos <- round(as.data.frame(prototype_dist_cos),2)
...
png("plots/category_prototype_cos_dist.png", 1400, 1400, type = 'cairo')
...
ggplot(cos_tidied,
  ...
  coord_fixed()+
  labs(title = "Cosine dissimilarity between category prototypes") +
  labs(x = "Category 1", y = "Category 2") +
...
We need to validate whether the embeddings distances actually match the categorization and can distinguish between furnishings:
```{r}
library(ProcMod)

closest_eucl <- data.frame(matrix(ncol = 0, nrow = 0))
dist_df_eucl <- as.data.frame(embeddings_dist_eucl)

closest_cos <- data.frame(matrix(ncol = 0, nrow = 0))
dist_df_cos <- as.data.frame(embeddings_dist_cos)


category_dist_eucl <- dist_df_eucl[endsWith(rownames(dist_df_eucl), "category"),]
literal_dist_eucl <- dist_df_eucl[grepl("literal",rownames(dist_df_eucl), fixed = TRUE),]
sentimental_dist_eucl <- dist_df_eucl[grepl("sentimental",rownames(dist_df_eucl), fixed = TRUE),]
image_dist_eucl <- dist_df_eucl[endsWith(rownames(dist_df_eucl), ".png"),]
model_dist_eucl <- dist_df_eucl[endsWith(rownames(dist_df_eucl), "mean"),]
...
for(col in 1:ncol(dist_df_eucl)){
  if(endsWith(rownames(dist_df_eucl)[col], "category")){
    next
  }
  # Consider the 3 nearest categories a confident decision.
  sorted_cat_comp_eucl <- sort(category_dist_eucl[,col], index.return=TRUE)
  nearest_cats_eucl <- rownames(category_dist_eucl)[head(sorted_cat_comp_eucl$ix[sorted_cat_comp_eucl$x>0],3)]
  ...
  sorted_model_comp_eucl <- sort(model_dist_eucl[,col], index.return=TRUE)
  nearest_models_eucl <- rownames(model_dist_eucl)[head(sorted_model_comp_eucl$ix[sorted_model_comp_eucl$x>0],5)]
  ...
  sorted_literal_comp_eucl <- sort(literal_dist_eucl[,col], index.return=TRUE)
  nearest_literal_eucl <- rownames(literal_dist_eucl)[head(sorted_literal_comp_eucl$ix[sorted_literal_comp_eucl$x>0],15)]
  ...
  sorted_sentimental_comp_eucl <- sort(sentimental_dist_eucl[,col], index.return=TRUE)
  nearest_sentimental_eucl <- rownames(sentimental_dist_eucl)[head(sorted_sentimental_comp_eucl$ix[sorted_sentimental_comp_eucl$x>0],15)]
  ...
  sorted_image_comp_eucl <- sort(image_dist_eucl[,col], index.return=TRUE)
  nearest_images_eucl <- rownames(image_dist_eucl)[head(sorted_image_comp_eucl$ix[sorted_image_comp_eucl$x>0], 24)]
  ...
  closest_eucl <- rbind(closest_eucl,c(nearest_cats_eucl,
                                       nearest_models_eucl,
                                       nearest_literal_eucl,
                                       nearest_sentimental_eucl,
                                       nearest_images_eucl))
...
rownames(closest_eucl) <- colnames(dist_df_eucl)[!endsWith(colnames(dist_df_eucl), "category")]
colnames(closest_eucl) <- cnames
...
```{r}
# Next, find a heuristic to determine whether an entry is correct:
# Category: is in three most similar cats (related to target cat)
# Model: is equal to target furnishing, in same category, or in nearest adjacent category.
# Literal: is equal to target furnishing or the guessed furnishing is among the five most related furnishings
# Sentimental: is equal to target furnishing or the guessed furnishing is among the five most related furnishings
# Image: is equal to target furnishing or the guessed furnishing is among the five most related furnishings or in the correct category.
for (r in rownames(closest_eucl)){
  print(r)
}
```
